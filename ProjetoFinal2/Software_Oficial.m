...
Kg_CF = Kg; % Kg com condicao de contorno p/ calculo das forças
...
for i = 1 : NumLoadedNodes
    ...
    displa = [U(GDL(i,[2 3]))']; %ver isso aqui
...
for i = 1 : Stuck_Nodes 
    ...
    ElemEqs = GDL(HDBCNodes(i, 1),HDBCNodes(i, 2)+1);
    
    if HDBCNodes(i, 2) == 1
        fprintf(OutFile, '%5d FX = %e\n', HDBCNodes(i, 1), Pg(ElemEqs));
    end
